...
import { tableNames } from "../src/lib/table-names";
...
async function main() {
  ...
  const tables = ["records", "users", "settings", "employees", "honorariums"] as const;
  try {
    ...
    const report = await transaction(async () => {
      ...
      for (const table of tables) {
        const destinationTable = tableNames[table];
        const rows = source.prepare(`SELECT * FROM ${table}`).all();
        for (const row of rows) {
          const key = table === "settings" ? "key" : "id";
          const old = await db.prepare(`SELECT * FROM ${destinationTable} WHERE ${key}=?`).get(row[key]);
          if (old) {
            if (JSON.stringify(old) !== JSON.stringify(row)) {
              ...
              await db.prepare(`UPDATE ${destinationTable} SET ${columns.map(column => `${column}=?`).join(",")} WHERE ${key}=?`).run(...columns.map(column => row[column]), row[key]);
            }
            continue;
          }
          const columns = Object.keys(row);
          await db.prepare(`INSERT INTO ${destinationTable} (${columns.join(",")}) VALUES (${columns.map(() => "?").join(",")})`).run(...Object.values(row));
        }
        if (baseline) {
          const key = table === "settings" ? "key" : "id";
          ...
          for (const previous of baseline.prepare(`SELECT * FROM ${table}`).all()) {
            ...
            const old = await db.prepare(`SELECT * FROM ${destinationTable} WHERE ${key}=?`).get(previous[key]);
            if (!old) continue;
            assert.deepEqual(old, { ...previous }, `Removed source row changed in destination: ${table}`);
            await db.prepare(`DELETE FROM ${destinationTable} WHERE ${key}=?`).run(previous[key]);
          }
        }
        counts[table] = rows.length;
        const key = table === "settings" ? "key" : "id";
        const persisted = await db.prepare(`SELECT * FROM ${destinationTable} ORDER BY ${key}`).all();
      ...
      for (const row of source.prepare("SELECT workspace,payload FROM records").all()) {
        ...
        for (const doc of trip.documents || []) {
          ...
          const old = await db.prepare("SELECT content FROM lampiran WHERE workspace=? AND id=?").get(row.workspace, doc.id);
          if (old) assert.deepEqual(old.content, bytes);
          else await db.prepare("INSERT INTO lampiran(workspace,id,content) VALUES(?,?,?)").run(row.workspace, doc.id, bytes);
          attachments++;
        }
      }
