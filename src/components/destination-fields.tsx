...
export default function DestinationFields({values, options, onChange, className = "", highlightFilled = false}: {
  ...
  return <div className={`destination-fields ${className}`} role="group" aria-label="Tujuan perjalanan">
    {values.map((value, index) => <div className="destination-row" key={rows.keys[index]}>
      ...
      {values.length > 1 && <Button type="button" variant="ghost" size="icon-sm" aria-label={`Hapus tujuan ${index + 1}`} onClick={() => { rows.remove(index); onChange(values.filter((_, currentIndex) => currentIndex !== index)); }}><X size={16} /></Button>}
    </div>)}
