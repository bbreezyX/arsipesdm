...
export default function HonorariumForm({ initial, employees, onClose, onSaved }: {
  ...
  return <>
    <Dialog open onOpenChange={open => { if (!open) close(); }}>
      <DialogContent className="form-dialog honor-dialog" showCloseButton={!busy} onInteractOutside={e => e.preventDefault()}>
        <DialogHeader>
          <div className="dialog-kicker">{kicker}</div>
          <DialogTitle>{editing ? "Edit honorarium" : "Tambah honorarium"}</DialogTitle>
          <DialogDescription>Satu rekap untuk satu penerima. Bruto, pajak, dan netto dihitung langsung dari isian, mengikuti format Lampiran 3.</DialogDescription>
        </DialogHeader>
        <form className="editor-form" onSubmit={save}>
          <div className="form-body"><fieldset disabled={busy} className="honor-fieldset">
          ...
          <div className="honor-calc" aria-live="polite">
            ...
            <div><span>Pajak{form.taxMode === "percent" ? ` ${form.taxRate.toLocaleString("id-ID")}% dari bruto` : ""}</span><strong>− {money(totals.tax)}</strong></div>
