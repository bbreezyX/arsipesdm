...
export async function GET() {
  try {
    ...
    return Response.json(getEmployees(c.workspace), {headers: {"Cache-Control": "private, no-store"}});
...
export async function POST(req: Request) {
  try {
    ...
    return Response.json(createEmployee(c.workspace, parsed.data), {status: 201});
  } catch (e) { return apiError(e); }
}
